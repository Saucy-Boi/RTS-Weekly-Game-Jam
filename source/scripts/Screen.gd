...
onready var base_size = Vector2(1280, 720)
...
func set_windowed():
	var window_size = OS.get_screen_size()
	## windowed version set to an arbitrary 50% of screen size here
	var scale = min(window_size.x / base_size.x, window_size.y / base_size.y) * 0.5
	var scaled_size = (base_size * scale).round()
